...
export default function NotificationBell() {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button className="bell" aria-label="Notifications" data-testid="notif-bell" onClick={() => setOpen((o) => !o)}>
      ...
      {open && (
        <div className="dropdown">
          <div className="row-between" style={{ padding: '12px 14px', borderBottom: '1px solid var(--hairline)' }}>
            ...
            {count > 0 && <button className="btn btn-ghost btn-sm" onClick={() => markAllRead.mutate()}>Mark all read</button>}
          </div>
          <div style={{ maxHeight: 360, overflowY: 'auto' }}>
            {items.length === 0 && <div className="muted small" style={{ padding: 16 }}>No notifications yet.</div>}
            {items.map((n) => (
              <button key={n.id} data-testid="notif-item"
                ...
                <div className="muted" style={{ fontSize: 11, marginTop: 3 }}>{fromNow(n.createdAt)}</div>
              </button>
            ))}
          </div>
        </div>
